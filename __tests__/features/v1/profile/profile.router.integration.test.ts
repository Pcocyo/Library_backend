...
describe.skip("Profile Route GET and PATCH endpoint test", () => {
    ...
    let initializeDummyProfile = () => {
        ...
        let profile = ProfileService.Tests__CreateProfile__({
            user_id: dummyId,
    ...
    let dummyUserToken:string;
    let dummyLibrarianToken:string;
    let dummyMemberUserToken:string;
    //profile variable
    ...
    beforeAll(() => {
        ...
        dummyLibrarianToken = jwtService.generateJwtToken({
            email: dummyLibrarian.getEmail(),
            id: dummyLibrarian.getId(),
            role: dummyLibrarian.getUserRole(),
        });
        dummyUserToken = jwtService.generateJwtToken({
            email: dummyUser.getEmail(),
            id: dummyUser.getId(),
            role: dummyUser.getUserRole(),
        });
        dummyMemberUserToken = jwtService.generateJwtToken({
            email: dummyMemberUser.getEmail(),
            id: dummyMemberUser.getId(),
            role: dummyMemberUser.getUserRole(),
        });
    ...
    it("PATCH /profile/librarian/update should return with CLIENT_ERROR_002 if user is not a librarian", async () => {
        const response = await request(serverApp).patch(
            "/profile/librarian/update",
        );
        expect(response.status).toBe(401);
        expect(response.body.code).toBe(ClientErrorCode.Unauthorized_Request);
        expect(response.body.name).toBe("CLIENT_ERROR");
    ...
    it("PATCH /profile/subscribe will throw an CLIENT_ERROR_006 if user is already a member", async () => {
        const response = await request(serverApp)
            .patch("/profile/subscribe")
            .set({ Authorization: dummyMemberUserToken });
        expect(response.status).toBe(400);
        expect(response.body).toHaveProperty("name");
        expect(response.body.name).toBe("CLIENT_ERROR");
        expect(response.body).toHaveProperty("code");
